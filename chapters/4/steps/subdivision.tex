...
\begin{algorithm}[h!]
	...
	\KwResult{A closed, orientable 3--dimensional polyhedral gluing $N'$ that is a subdivision of $N$}
	\Begin{
...
%		$G_k = $ the secants of the regular $k$-gon inscribed in the circle with one corner at $(-1,0)$\;
		...
		$N' = $ the set of 3--complexes $\tau$ constructed above, glued together as prescribed by the gluing maps of $N$ and the identification of the $\tau$ as subdivisions of the tetrahedra of $N$\;
	}
\end{algorithm}
...
\begin{algorithm}[h!]
	...
	\KwData{A closed, orientable 3--manifold $N'$ expressed as a 3--dimensional polyhedral gluing}
	\KwResult{A closed, orientable 3--manifold triangulation $M$ that is a subdivision of $N'$}
	\Begin{
		$M = N'$\;
		\ForEach{polyhedral 3--cell $\tau$ of $M$ that is not a tetrahedron}{
			Replace $\tau$ with the cone on $\pd \tau$\;
		}
		\ForEach{polygonal 2--cell $\delta$ of $M$ that is not a triangle}{
			$\delta$ is the shared boundary of the polyhedral 3--cells $\tau_+$ and $\tau_-$\;
			Each of $\tau_+$, $\tau_-$ contains one vertex that is disjoint from $\delta$\;
			Call these vertices $v_+$, $v_-$ respectively\;
			Delete $\tau_+$ and $\tau_-$ from $M$\;
			Subdivide $\delta$ using $|\delta^0|-2$ triangles\;
			\ForEach{new 1--cell $e$ in the triangulation of $\delta$}{
				Attach triangles over $e$ and $v_+$ and over $e$ and $v_-$\;
			}
			\ForEach{triangle $f$ in the triangulation of $\delta$}{
				Attach tetrahedra over $f$ and $v_+$ and over $f$ and $v_-$\;
			}
		}
	}
\end{algorithm}
...
\begin{algorithm}[h!]
	...
	\KwData{A closed, orientable 3--manifold triangulation $M$}
	\KwResult{A 4--manifold triangulation $W$ that is exactly $M\times I$}
	\Begin{
		$W=\{P_\tau\;|\;\tau\in M^3\}$, where each $P_\tau$ is a 4--prism with identical walls\;
		\ForEach{triangle $\delta\in M^2$}{
			$\delta$ is the shared boundary of the tetrahedra $\sigma$ and $\tau$\;
			Attach the prisms $P_\sigma$ and $P_\tau$ of $W$ to one another over the walls of each that correspond to $\delta$\;
		}
	}
\end{algorithm}
...
Each edge $e$ of $N$ is mapped to a secant of the circle, splitting the disc into exactly 2 sectors: the `left' and `right' sides of $f(e)$.
...
%	observe that the topology of a surface above a transversal $\gamma$ traveling along the secant $f(e)$ changes only when $\gamma$ encounters a vertex sleeve, so we need only examine the surfaces $f\inv(\gamma)$ when $\gamma$ lies on the boundary of a vertex sleeve.
%	Let
