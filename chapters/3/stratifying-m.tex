...
\begin{proof}[Proof of Theorem \ref{thm:block-structure}]
	...
	Call the arcs $\gamma_1$ and $\gamma_2$ and let $x = \gamma_1\cap \gamma_2$.
	Our method of investigation begins by examining the possible resolutions of the interactive singular fiber above $x$ and combining those resolutions to form a genus 3 surface.
	 
	Figure \ref{fig:codim-2-interactive-fiber-1} demonstrates resolutions of the singular points in the first interactive fiber.
	...
	\begin{figure}[h!]
		\caption{
			...
			The fiber and its possible resolutions over nearby codimension 1 singular values and regular values.
	...
	\begin{figure}[h!]
		\caption{
			...
			The surface is presented so that its construction from arc preimage subsurfaces is clear.
		}
		\label{fig:codim-2-surface-1}
	\end{figure}
	
	The surface $\Sigma$ in Figure \ref{fig:codim-2-surface-1} is the boundary of the submanifold of $M$ that projects over a tubular neighbourhood of $x$, and $\Sigma$ projects over $\pd(\nu(x))$.
	$\Sigma$ is illustrated as a genus-3 orientable surface embedded in $S^3$, realized as the one-point compactification of $R^3$, such that $\Sigma$ bounds a genus-3 (3,1)--handlebody on both sides (i.e. $S^3\setminus\Sigma$ is a pair of genus-3 (3,1)-handlebodies).
	$f\inv(x)$ is illustrated as `outside' of the surface to demonstrate its relationship with $f\inv(\nu(x))$, but this positioning does not make the relationship between $f\inv(x)$ and $\Sigma$ immediately clear.
	
	\begin{figure}[h!]
		\caption{
			\textbf{Surface $\Sigma$ near the first interactive fiber that projects over $\pd\nu(x)$, inverted.}
			This figure is equivalent to Figure \ref{fig:codim-2-surface-1} under an inverse transformation of the $S^3$ in which $\Sigma$ is embedded.
		}
		\label{fig:codim-2-surface-1-inv}
	\end{figure}
	
	Figure \ref{fig:codim-2-surface-1-inv} illustrates Figure \ref{fig:codim-2-surface-1} as transformed by inversion of $S^3$, and the relationship between $f\inv(x)$ and $\Sigma$ is now clear.
