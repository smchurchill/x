...
\begin{proof}[Proof of Theorem \ref{thm:block-structure}]
	...
	%% The surface displayed is embedded in S^3

	\begin{figure}[h!]
	\caption{
		\textbf{Surface near the first interactive fiber that projects over $\pd\nu(x)$ }
		The fiber and its possible resolutions over nearby codimension 1 singular values and regular values.
		The fibers inherit orientation from $M$, and this illustration is presented without loss of generality.
	}
...
\end{figure}
	...
	, and each vertex region was created by fitting with an octagon around an intersection of $X_f$.
	Let $\gamma_1$ and $\gamma_2$ be the arcs of codimension 1 singular values that intersect inside of $R$, and let $x=\gamma_1\cap\gamma_2$.	
	We have explored the possible surfaces that can project through $f$ over the edges of $R$: cylinders project over the edges of $R$ shared with face regions, and pairs of cylinders, discs, or pairs of pants may project over the edges of $R$ shared with edge regions.
	This is illustrated in Figure \ref{fig:codim-2-surfaces}.
	
	\begin{figure}[h!]
		\caption{
			\textbf{Surfaces on the boundary of vertex regions.}
			note
		}
		\label{fig:codim-2-surfaces}
	\end{figure}
	...
	\begin{figure}[h!]
		\caption{
			\textbf{Interactive blocks.}
			nope
		}
		\label{fig:codim-2-block}
	\end{figure}
